        ...
        static void NoteLogMessageForDrawingAtGameObjects(LogType logType, GameObject gameObject_toWhichLogMessageBelongs, string message, string stackTrace = null)
        {
            if (gameObject_toWhichLogMessageBelongs != null)
            {
                if (message != null)
                {
                    ...
                    //setting a placeholder here because Unity deprecated "GetInstanceID()" and "GetEntityID()" is not yet a reliable replacement
                    receivedLogMessage.gameObjectsInstanceID = 0;
#else
...
#if UNITY_6000_5_OR_NEWER
            //setting a placeholder here because Unity deprecated "GetInstanceID()" and "GetEntityID()" is not yet a reliable replacement
            string logsAsTextWall = "Drawing logs at game objects is tentatively deprecated in Unity 6.5 and newer, because Unity deprecated the neccessary function object.GetInstanceID(), and object.GetEntityID() isn't yet a reliable replacement. It might get reactivated in the future.";
#else
