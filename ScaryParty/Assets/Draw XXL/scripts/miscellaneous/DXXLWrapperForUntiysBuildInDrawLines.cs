            ...
            if (drawnLinesSinceFrameStart >= DrawBasics.MaxAllowedDrawnLinesPerFrame)
            {
                if (maxLinesPerFrameWarningTextDraw_hasAlreadyBeenStarted_duringCurrFrame == false)
                {
                    ...
                    try
                    {
                        ...
                        //the following line has a deprecation warning and the IDE suggests to replace "FindObjectsByType" with an overload without the "FindObjectsSortMode"-paramter. Though this overload is not yet documented for any available Unity version (on 20.05.2026 checked for Unity 6.3, 6.4, 6.5 beta, 6.6 alpha). Therefore too risky to implement now.
                        if (activeCamerasOfTheScene == null) { activeCamerasOfTheScene = UnityEngine.Object.FindObjectsByType<Camera>(FindObjectsSortMode.InstanceID); }
#else
